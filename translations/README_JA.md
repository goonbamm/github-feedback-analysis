...
- 🎊 **年間振り返り**：今年活動したすべてのリポジトリを自動分析し、総合的な年次レポートを生成
...
- ⚡ **ネットワーク最適化**：GitHub APIリクエストをキャッシュ（デフォルト有効期限は1時間）し、LLM呼び出しに自動リトライと指数バックオフを適用
...
# 今年の活動を自動分析（Year in Review）
gfa feedback -y
```

推奨リポジトリのリストから選択するか、直接入力して**あなたの活動**を分析できます。`-y` オプションを使うと、今年アクティブだったすべてのリポジトリをまとめて年間レビューを即座に生成でき、リポジトリを手動で選ぶ必要がありません。

分析完了後、`reports/`ディレクトリに以下のファイルが生成されます：
- `metrics.json` - 分析データ
- `report.md` - Markdownレポート
- `integrated_full_report.md` - **統合レポート**（リポジトリ分析 + PRレビューを統合）
- `prompts/` - LLMプロンプトファイル
- `reviews/` - 個別PRレビューと個人の成長分析
...
# 🎊 年間振り返りレポート（今年活動したすべてのリポジトリの総合分析）
...
├── report.md                        # 📄 Markdownレポート
├── integrated_full_report.md        # 🎯 統合レポート（brief + PRレビュー）
├── reviews/                         # 🔍 PRレビュー（サブディレクトリ）
│   └── {owner}__{repo_name}/        # リポジトリ別レビュー（owner__repo形式）
│       ├── pr-{number}/
│       │   ├── artefacts.json       # 元のPRデータ
│       │   ├── review_summary.json  # 構造化されたレビュー
│       │   └── review.md            # Markdownレビュー
│       ├── integrated_report.md     # 統合PRレビューレポート
│       └── personal_development.json  # 個人成長分析
└── year-in-review/                  # 🎊 年次振り返り（--year-in-reviewオプション使用時）
    ├── year_{year}_in_review.md     # 年次総合レポート
    └── metrics/                     # リポジトリ別の詳細メトリクス
        └── {owner}__{repo}.json     # 各リポジトリの詳細スナップショット
```
...
- **リトライロジック**：LLMリクエストに指数バックオフを適用（最大5回試行、2秒から倍々で増加）
