...
- 🎨 **交互模式**：用户友好的仓库直接选择界面
- ⚡ **网络优化**：缓存 GitHub API 请求（默认 1 小时过期），对 LLM 调用应用自动重试和指数退避
...
您可以从推荐仓库列表中选择或直接输入仓库来分析**你的活动**。使用 `-y` 选项，可立即生成汇总今年所有活跃仓库的年度回顾，无需手动选择仓库。

分析完成后，将在 `reports/` 目录中生成以下文件：
- `metrics.json` - 分析数据
- `report.md` - Markdown 报告
- `integrated_full_report.md` - **综合报告**（合并仓库分析 + PR 审查）
- `prompts/` - LLM 提示文件
- `reviews/` - 单个 PR 审查和个人成长分析
...
- **重试逻辑**：LLM 请求采用指数退避（最多 5 次尝试，从 2 秒开始逐步加倍）
