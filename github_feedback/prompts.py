...
def get_personal_development_system_prompt() -> str:
    ...
    return (
        ...
        "- **evidence**: PR 번호와 함께 실제 관찰된 내용을 **정량적 정보와 함께** 포함 (예: '총 10개 PR 중 7개에서 관찰됨', "
        "실제 PR 제목 인용, PR 설명 내용의 핵심 부분 인용, 리뷰 코멘트의 실제 문장 인용, 코드 변경량 정보 포함)\n"
        "- **suggestions**: '테스트를 추가하세요' 같은 일반적인 조언이 아닌, 실행 가능한 구체적 액션 아이템 (예: "
        "'다음 5개 PR에서 PR 설명에 (1)변경 이유 (2)구현 방법 (3)테스트 방법을 모두 포함하기')\n\n"
        "**응답 형식 (JSON):**\n"
        "{\n"
        '  "tldr_summary": {\n'
        '    "top_strength": "가장 뛰어난 강점 1개 (간결하게)",\n'
        '    "primary_focus": "이번 달 집중할 핵심 개선 사항 1개 (구체적이고 실행 가능한)",\n'
        '    "measurable_goal": "측정 가능한 목표 (예: \'PR 제목 명확성 70% → 90%\', \'리뷰 응답 시간 3일 → 1일\')"\n'
        '  },\n'
        '  "strengths": [\n'
        "    {\n"
        ...
        '      "evidence": [\n'
        '        "총 10개 PR 중 8개(80%)에서 명확한 컨벤션 prefix 사용 관찰됨",\n'
        '        "PR #123 제목: \'feat: 인증 시스템에 JWT 토큰 검증 로직 추가\' - 명확한 컨벤션 prefix와 핵심 변경사항 포함",\n'
        '        "PR #145 설명 인용: \'기존 세션 방식은 확장성 문제가 있어 JWT로 전환합니다. jsonwebtoken 라이브러리를 선택한 이유는...\' - 변경 이유와 구현 방법을 상세히 기술",\n'
        '        "PR #167 리뷰 코멘트 인용: \'이 부분은 edge case를 고려하면 null check가 필요할 것 같아요. 예를 들어 토큰이 만료된 경우...\' - 구체적인 문제와 예시를 함께 제시",\n'
        '        "PR #189 코드 변경: +350/-120줄, 복잡한 인증 로직을 validateToken, refreshToken, revokeToken 3개 함수로 분리하여 테스트 커버리지 85%로 증가"\n'
        '      ],\n'
        '      "impact": "high|medium|low"\n'
        "    }\n"
        "  ],\n"
        ...
        '      "evidence": [\n'
        '        "총 10개 PR 중 3개(30%)에서 모호한 제목 사용 관찰됨",\n'
        '        "PR #134 제목: \'Update API\' - 어떤 API를 어떻게 수정했는지 파악 불가, 개선 필요",\n'
        '        "PR #156 설명: \'버그 수정\'으로만 작성 - 어떤 버그를 어떻게 해결했는지 불명확, 리뷰어가 추가 질문 필요",\n'
        '        "PR #178 리뷰 코멘트 인용: \'이거 고쳐주세요\' - 구체적인 이유나 제안 없이 요청만 하여 협업 효율 저하, 왕복 커뮤니케이션 2-3회 발생",\n'
        '        "PR #192 코드 변경: +800/-200줄의 대규모 변경, API 응답 필드를 기존 클라이언트 호환성 고려 없이 직접 수정하여 배포 시 장애 위험"\n'
        '      ],\n'
        '      "suggestions": [\n'
        ...
        "    {\n"
        '      "week": 1,\n'
        '      "action": "구체적인 주간 실행 과제 (예: \'다음 5개 PR 제목에 feat/fix/refactor prefix 적용하기\')",\n'
        '      "measurable_goal": "측정 가능한 목표 (예: \'PR 제목 명확성 점수 7→8점 달성\')"\n'
        "    },\n"
        "    ...\n"
        "  ],\n"
        '  "progress_metrics": [\n'
        "    {\n"
        '      "area": "평가 영역 (예: \'PR 제목 명확성\', \'PR 설명 완성도\', \'리뷰 응답 속도\')",\n'
        '      "current_score": 7.0,\n'
        '      "target_score": 9.0,\n'
        '      "unit": "점"\n'
        "    },\n"
        ...
        '  "benchmarks": [\n'
        "    {\n"
        '      "metric": "비교 지표 (예: \'PR 리뷰 응답 시간\', \'평균 코드 변경 크기\', \'PR 당 토론 횟수\')",\n'
        '      "my_value": "나의 값 (예: \'2.5일\', \'평균 +250/-150줄\', \'5회\')",\n'
        '      "team_average": "팀 평균 (예: \'1.8일\', \'평균 +200/-100줄\', \'8회\')",\n'
        '      "recommendation": "평가 (\'개선 필요\', \'양호\', \'우수\' 중 하나)",\n'
        '      "insight": "추가 설명이나 제안 (선택사항)"\n'
        "    },\n"
        "    ...\n"
        "  ]\n"
        "}\n\n"
        ...
        "- strengths, improvement_areas, growth_indicators: 각 배열은 최소 1개, 최대 5개 항목\n"
        "- action_plan: 4주간의 주간 계획 (4개 항목)\n"
        "- progress_metrics: 2-4개의 핵심 메트릭\n"
        "- benchmarks: 2-4개의 비교 지표\n\n"
        "**Few-shot 예제 (좋은 응답 예시):**\n\n"
        "좋은 strength 예시:\n"
        "```json\n"
        "{\n"
        '  "category": "일관된 컨벤션을 따르는 명확한 PR 제목 작성으로 코드 리뷰 효율성 향상",\n'
        '  "description": "모든 PR 제목에 feat/fix/refactor prefix를 일관되게 사용하여 변경 유형을 명확히 전달합니다. '
        "특히 제목만으로도 변경 범위와 영향을 파악할 수 있도록 구체적으로 작성하는 습관이 돋보입니다. "
        '이로 인해 리뷰어가 PR을 빠르게 이해하고 효율적으로 리뷰할 수 있습니다.",\n'
        '  "evidence": [\n'
        '    "총 15개 PR 중 13개(87%)에서 명확한 컨벤션 prefix 사용 관찰됨",\n'
        '    "PR #245 제목: \'feat: 사용자 인증에 JWT 토큰 검증 로직 추가\' - 변경 유형과 핵심 내용이 명확",\n'
        '    "PR #267 제목: \'fix: 결제 API 타임아웃 시 재시도 로직 추가하여 안정성 개선\' - 문제와 해결책이 명확히 드러남",\n'
        '    "PR #289 설명 인용: \'기존 세션 기반 인증은 확장성 문제로 JWT로 전환. jsonwebtoken 라이브러리 선택 이유는...\' - 변경 이유와 구현 방법 상세 기술",\n'
        '    "코드 변경: 평균 +280/-150줄, 테스트 커버리지 평균 82%로 높은 품질 유지"\n'
        "  ],\n"
        '  "impact": "high"\n'
        "}\n"
        "```\n\n"
        "나쁜 strength 예시 (이렇게 작성하지 마세요):\n"
        "```json\n"
        "{\n"
        '  "category": "코드 품질",  // ❌ 너무 일반적\n'
        '  "description": "코드를 잘 작성합니다.",  // ❌ 구체성 부족, 1문장만\n'
        '  "evidence": [  // ❌ 정량적 데이터와 실제 인용 없음\n'
        '    "좋은 PR들이 많음",\n'
        '    "코드가 깔끔함"\n'
        "  ],\n"
        '  "impact": "high"\n'
        "}\n"
        "```\n\n"
        "좋은 improvement_area 예시:\n"
        "```json\n"
        "{\n"
        '  "category": "PR 설명에 변경 이유와 테스트 계획 보완 필요",\n'
        '  "description": "PR 제목은 명확하지만 설명 부분이 간략하여 리뷰어가 변경의 맥락과 테스트 방법을 파악하기 어렵습니다. '
        "특히 \'왜\' 이 변경이 필요한지, 어떻게 테스트했는지에 대한 정보가 부족하여 리뷰 과정에서 추가 질문이 발생하고 있습니다. "
        '이는 리뷰 시간을 늘리고 왕복 커뮤니케이션을 증가시킵니다.",\n'
        '  "evidence": [\n'
        '    "총 15개 PR 중 6개(40%)에서 간략한 설명만 제공 (50자 미만)",\n'
        '    "PR #234 설명: \'버그 수정\'으로만 작성 - 어떤 버그를 어떻게 해결했는지 불명확",\n'
        '    "PR #256 리뷰 코멘트: \'이 변경이 왜 필요한가요?\' - 리뷰어가 추가 질문, 왕복 커뮤니케이션 2회 발생",\n'
        '    "PR #278 설명: \'API 업데이트\' - 구체적인 변경 내용과 영향 범위 누락, 리뷰 시간 평균 3.5일로 팀 평균(1.8일)보다 길어짐"\n'
        "  ],\n"
        '  "suggestions": [\n'
        '    "PR 설명에 최소한 (1)변경 이유 (2)구현 방법 (3)테스트 방법 세 가지를 포함하여 리뷰어의 이해를 돕기",\n'
        '    "다음 5개 PR에서 \'왜 이 변경이 필요한가?\'와 \'어떻게 테스트했는가?\'를 명시적으로 기술하기",\n'
        '    "템플릿 활용: ## 변경 이유 / ## 구현 방법 / ## 테스트 계획 섹션을 PR 설명에 포함"\n'
        "  ],\n"
        '  "priority": "important"\n'
        "}\n"
        "```\n\n"
        "**중요 리마인더:**\n"
        "1. category는 반드시 구체적이고 상세하게 (10자 이상)\n"
        "2. description은 2-3문장으로 패턴과 영향 설명\n"
        "3. evidence는 정량적 데이터(빈도, 비율)와 실제 인용 포함\n"
        "4. 모든 evidence 항목에 PR 번호 포함"
    )
...
def get_personal_development_user_prompt(
    ...
    return (
        ...
        "**중요:** 각 영역에서 다음 정보를 evidence에 **반드시** 포함하세요:\n"
        "1. **정량적 데이터**: '총 N개 PR 중 X개에서 관찰됨' 형식으로 빈도 명시\n"
        "2. **실제 인용**: PR 제목, PR 설명, 리뷰 코멘트의 실제 문장을 따옴표로 인용\n"
        "3. **코드 변경량**: additions/deletions 정보 포함 (예: '+350/-120줄')\n"
        "4. **구체적 영향**: 해당 패턴이 어떤 결과를 초래했는지 명시 (예: '테스트 커버리지 85%로 증가', '왕복 커뮤니케이션 2-3회 발생')\n\n"
        "추상적인 평가가 아닌 관찰된 사실과 측정 가능한 데이터에 기반한 분석을 작성하세요."
    )
